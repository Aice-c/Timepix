...
B2 废弃原“主干/结构迁移验证”定位，不再重新比较 backbone。Proton_C_7 只有 `ToT`，因此 B2 固定使用 B1-best patience=8 配置，只验证 Alpha A5 中有效的 `ToT-only handcrafted features` 是否能迁移到 Proton_C_7。
