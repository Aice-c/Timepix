...
- P1a `ps3_totgmmk2_v1 ToT-only seed42` 已完成，`Val/Test Macro-F1` 约 `0.977/0.978`，证明新提纯数据集上 ToT 单模态已很强；P1lr 已完成，`3e-5` 在性能接近最高分的同时显著减少 validation collapse，后续 P1 模态诊断默认使用 `learning_rate=3e-5`、`epochs=40`、`early_stopping_patience=12`。
