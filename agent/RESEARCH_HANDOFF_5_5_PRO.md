...
- A5 物理/手工标量特征融合方案已进入实现：不参考 `timepix/analysis/` 的既有特征实现；训练链路已新增 12 维合理保留候选特征、`handcrafted_features.source_modalities` 解耦、`handcrafted_mlp` 和 `scripts/screen_handcrafted_features.py`。A5a 用 `RandomForest` / one-vs-rest `LogisticRegression` / validation permutation importance 做筛选诊断；其中 `LogisticRegression` 显式使用 `OneVsRestClassifier(LogisticRegression(solver="liblinear"))` 以兼容服务器 `scikit-learn` 多分类行为。后续只做少量 A5b/A5c/A5d CNN 融合验证，避免全特征大网格。A5 同时区分可迁移 `ToT-only` 特征线与 Alpha-only `ToT+ToA` 特征线。
