...
- A4b-4e：三 seed 确认。已新增 `configs/experiments/a4b_4e_relative_minmax_no_mask_seed43_44.yaml`，只补跑 `relative_minmax/no mask` candidate 的 seed43/44；seed42 candidate 复用 `a4b_toa_transform_seed42`，ToT 三 seed 复用 `a2_best_3seed`。新增 `scripts/aggregate_selector_fusion.py` 用于汇总 selector 三 seed mean/std。
- A4b-5：entropy soft gate，把 A4b-4a hard switch 改为 validation-selected sample-wise soft interpolation。
- A4b-5 已开始实现为正式 gated late fusion 系列：`scripts/evaluate_gated_late_fusion.py` 同时比较 entropy soft gate、learned scalar probability/logit gate、class-aware gate 和 conservative gate；ResNet experts 冻结，validation 选 gate，test 只报告。
- A4b-6 已实现为正式 residual gated fusion 系列：`scripts/evaluate_residual_gated_fusion.py` 固定 ToT 为主模型，比较 scalar beta、per-class beta、learned gate+beta 和 conservative residual correction；ResNet experts 冻结，validation 选择 residual 方案，test 只报告。
