...
- A5 物理/手工标量特征融合方案已进入实现：不参考 `timepix/analysis/` 的既有特征实现；训练链路已新增 12 维合理保留候选特征、`handcrafted_features.source_modalities` 解耦、`handcrafted_mlp` 和 `scripts/screen_handcrafted_features.py`。A5a 已完成，用 `RandomForest` / one-vs-rest `LogisticRegression` / validation permutation importance 做筛选诊断；其中 `LogisticRegression` 显式使用 `OneVsRestClassifier(LogisticRegression(solver="liblinear"))` 以兼容服务器 `scikit-learn` 多分类行为。A5a 显示 Geometry 最重要，其次是 ToT；ToA 标量有补充但不是主导。A5b 低冗余 seed42 CNN concat pilot 已完成；准确口径是 `Geometry -> Geometry+ToT -> Geometry+ToT+ToA` 递进主线，另设 `ToA-only` 诊断组。A5b 没有证明 handcrafted concat 能稳定提升 Alpha ToT CNN；`geometry_lowcorr` 仅 MAE 小幅改善，`toa_lowcorr_diagnostic` 改善 30 deg recall 但 test accuracy 下降。A5c 已新增正式配置 `configs/experiments/a5c_alpha_handcrafted_gated_seed42.yaml`，镜像 A5b 的四个特征组并只把融合方式改为 `gated`；A5c 不重复 concat，也不运行 handcrafted-only MLP，除非论文后续需要额外对照。因此 B2 不再表述为“迁移 Alpha 最优手工特征”，而是低成本验证 Proton_C_7 上 ToT-only 标量是否与 CNN 冗余或能改善弱类别。
