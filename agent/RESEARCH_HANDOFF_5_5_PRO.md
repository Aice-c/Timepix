...
- A4c-4 配置已新增：`configs/experiments/a4c_warm_started_expert_gate.yaml` 与 `configs/experiments/a4c_warm_started_expert_gate_seed42.yaml`。完整三 seed 为 6 个 run：`warm_started_expert_gate × freeze_experts true/false × seed 42/43/44`，服务器建议使用 `tmux new -s a4c_warm_gate` 持久化运行。
