...
- A5 物理/手工标量特征融合方案已进入实现：不参考 `timepix/analysis/` 的既有特征实现；训练链路已新增 12 维合理保留候选特征、`handcrafted_features.source_modalities` 解耦、`handcrafted_mlp` 和 `scripts/screen_handcrafted_features.py`。A5a 已完成，用 `RandomForest` / one-vs-rest `LogisticRegression` / validation permutation importance 做筛选诊断；其中 `LogisticRegression` 显式使用 `OneVsRestClassifier(LogisticRegression(solver="liblinear"))` 以兼容服务器 `scikit-learn` 多分类行为。A5a 显示 Geometry 最重要，其次是 ToT；ToA 标量有补充但不是主导。A5b 低冗余 seed42 CNN concat pilot 已完成；准确口径是 `Geometry -> Geometry+ToT -> Geometry+ToT+ToA` 递进主线，另设 `ToA-only` 诊断组。A5b 没有证明 handcrafted concat 能稳定提升 Alpha ToT CNN；`geometry_lowcorr` 仅 MAE 小幅改善，`toa_lowcorr_diagnostic` 改善 30 deg recall 但 test accuracy 下降。A5c gated seed42 已完成，显示 gated 明显优于 simple concat；五维 `geometry+ToT+ToA gated` 是当前 A5 最好结果（Test Acc 71.07%，MAE 5.651，Macro-F1 0.652），但仍是小幅提升，主要改善 45/60 deg，未解决 30 deg。A5d 已新增正式配置 `configs/experiments/a5d_alpha_handcrafted_gated_3seed.yaml`，包含五维 `geometry+ToT+ToA gated` main 和 `ToA-only gated` diagnostic 两组三 seed；这样既验证物理完整低冗余组，也保留 A5c validation-best ToA 标量诊断。因此 B2 不再表述为“迁移 Alpha 最优手工特征”，而是低成本验证 Proton_C_7 上 ToT-only 标量是否与 CNN 冗余或能改善弱类别。
...
- B3a Proton_C_7 有序损失 seed42 screening 已完成代码与配置适配，配置为 `configs/experiments/b3a_proton_c7_ordinal_loss_seed42.yaml`。B3a 固定 B1-best patience=8、ToT-only、无 handcrafted，只比较 loss / label strategy：Gaussian soft-target CE (`sigma=5/10/15`)、`ce_expected_mae` (`lambda=0.05/0.10/0.20`)、`ce_emd` (`lambda=0.05/0.10/0.20`)。不做 pure EMD，因为 Proton_C_7 的 CE baseline 已有很高 exact accuracy，pure EMD 可能让输出分布变宽并降低精确分类。代码新增真正的 soft-target CE、CE+expected-angle MAE、CE+angle-weighted CDF/EMD，并在 summary 中记录 high-angle Macro-F1、45/50 与 60/70 混淆、far-error rate。B3a 运行后只用 validation 选择是否进入 B3b 三 seed。
...
- A4b-5 已完成三 seed：validation-selected gate 将 Test Acc 从 ToT primary 的 70.44±0.15% 提高到 72.17±1.72%，MAE 从 5.949±0.068 降到 5.661±0.320，Macro-F1 从 0.636±0.009 提高到 0.662±0.027。它是当前 A4b 系列最强的实际选择性融合结果；但 seed43 选中了 `train` fit，论文中需说明偏乐观风险，保守口径可重点报告 `class_aware_prob_val-cv`。
- A4b-6 已完成三 seed：validation-selected residual 将 Test Acc 提高到 71.44±1.01%，MAE 降到 5.800±0.077，Macro-F1 提高到 0.656±0.010。它优于 ToT baseline，但弱于 A4b-5，更适合作为 residual-fusion 对照实验。
- A4c 第一阶段已完成 9 个 run。三 seed 汇总：`dual_stream_concat_aux` 为 Val Acc `69.90±1.34%`、Test Acc `72.10±1.35%`、Test MAE `5.631±0.333`、Macro-F1 `0.686±0.017`；`dual_stream_gmu_aux` 为 Val Acc `70.20±0.67%`、Test Acc `71.94±0.51%`、Test MAE `5.721±0.009`、Macro-F1 `0.691±0.009`；`toa_conditioned_film` 为 Val Acc `70.43±0.95%`、Test Acc `71.60±1.21%`、Test MAE `5.775±0.236`、Macro-F1 `0.678±0.021`。结论：A4c 没有显著超过 A4b-5 的 Test Acc，但提升了 Macro-F1 和 `30 deg` 困难类别 recall；论文解释上优先讲 GMU。
