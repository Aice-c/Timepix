...
日期：2026-09-09。本文件记录最初的环境、代码和数据准备，以下“未训练”仅指部署验收时点。用户随后授权的四组训练已完成；实际执行、结果和资源收尾见`CARBON_CONTROLS_RUNBOOK.md`及实验日志。

训练后收尾：四组完整结果回传并通过SHA核验后，服务器在最终文档Git拉取时SSH拒绝连接。最后确认服务器HEAD为`a8bbf56`，最终文档已推送隔离分支但尚未确认服务器拉取。断连前最后资源快照为GPU空闲、数据盘约41.95GiB；不作为当前状态。连接恢复后只需核对实例身份、无训练进程和工作区状态，再`git pull --ff-only`同步同分支文档，不需要重跑四组训练。
