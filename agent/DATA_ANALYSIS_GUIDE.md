...
- 数据集分析：说明原始 `256 x 256` Timepix3 帧如何经过连通区域轨迹提取、ToT 统计量清洗，形成 `Alpha_100` 和 `Proton_C` 两个监督学习数据集。
- 近垂直分辨极限分析：只针对 `Proton_C` 的 `ToT` 单模态，分析 `80, 82, 84, 86, 88, 90` 六个近垂直角度在当前数据表示和已测试特征/模型族下是否具备足够可分性。
