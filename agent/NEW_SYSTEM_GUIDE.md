...
B3a 结果需要和 B1-best patience=8 三 seed baseline 对照。B3a 只做 seed42 screening，不运行 `aggregate_seeds.py`。B3a 已完成，按 validation accuracy 选择 `CE+ExpectedMAE lambda=0.05` 进入 B3b-main；`CE+EMD lambda=0.05` 保留为 optional 对照。
