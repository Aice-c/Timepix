...
## 2026-09-16 T7普通卷积稳定性诊断与恢复Acc选模

状态：全部完成。运行提交8bd2c3c，本地/服务器45项测试及独立代码复核通过；Laplace执行诊断07:03:32至07:04:00、唯一S42训练/汇总07:23:23至07:56:31（2026-09-16北京时间），均exit0。Tesla完整回传101文件/27checkpoint/1,406,274,838bytes，双端SHA一致、初诊39文件保护通过。主控隔离分支`codex/carbon-stability-55870`，未动主工作区；无test、无重跑、无额外种子/同期复现、服务器保留开机。

关键决策顺序：用户同意“旧checkpoint诊断→降低学习率对照”，随后明确取消`3e-4`同期复现，因此只新增普通卷积`1e-4/seed42`一组。用户询问历史选模规则后确认**恢复Val Acc优先，其次Val MAE、Val Macro-F1，再取更早epoch；早停同规则**。旧B1/B2/B3是Acc优先，9月9日新T7/V6及9月15日Astra差分方案曾改为MAE优先；历史结果保持原口径，不回写。向网页端Astra交付时必须说明原则变化。

本轮来源R42 best3/last11，只比较FP32/AMP和模型副本train-only BN统计重估，不反向传播、不使用test、不覆盖旧模型。训练固定原T7分组manifest、R42标准化统计、ToT50×50、CE、Adam/wd1e-4/batch128/dropout0.1/25epoch/patience8/AMP/四旋转视图，只有lr降到1e-4以及上述选模规则恢复。新增逐batch实际optimizer步数/scale/梯度范数、逐epoch权重和BN摘要，默认关闭、不改旧训练行为。旧R42没有epoch6/9权重，不能完整还原崩塌现场。

重要限制：本轮取消同期原lr复现，且新旧比较同时改变学习率与选模/早停，故不把全部改善归因于LR，也不作为严格单变量结构优劣结论。BN重估分数只作诊断，不替代正式模型。先由主控审核诊断，再放行唯一训练。具体固定设置、完整诊断/训练/汇总/回传命令见`CARBON_STABILITY_RUNBOOK.md`。

部署前审查发现并修复诊断配置schema漏项、诊断目录并发占用、训练配置锁外写入、汇总身份/逐batch完整性校验不足；旧文件哈希在汇总时再次复核。45项本地及服务器测试通过。

诊断独立复核通过：旧best AMP为88.1321%，与旧六项指标差0；FP32为87.5343%。last AMP原BN66.9541%，train-only BN副本90.1607%；该副本FP32为84.4081%，两精度预测分歧17.58%，其中518条不是两精度下top2 logit差≤1的近tie。best的BN重估却降至AMP75.6174%。因此不把BN校准视为正式修复，保留BN/精度双重敏感性。旧文件SHA不变、非BN状态保持、optimizer步数0、test未评估。无数据/计算完整性阻断项，继续原批准的低LR诊断，不能保证其解决数值敏感性。

| 正式模型 | 选模 | best/stop | Val Acc | Val Balanced Acc | Val Macro-F1 | Val Weighted-F1 | Val MAE / ° |
| --- | --- | --- | ---: | ---: | ---: | ---: | ---: |
| R42历史lr3e-4 | MAE-first | 3/11 | 88.1321% | 91.2522% | 0.913131 | 0.878776 | 1.115739 |
| S42 lr1e-4 | Acc-first | 25/25 | 93.2771% | 95.1391% | 0.951184 | 0.932747 | 0.634065 |

S42未早停，正式与MAE回溯均选25，但patience历史不同；未来碳离子新配置显式Acc优先，旧公共配置保留历史MAE。64,200batch连续完整，64,171次实际更新与66个Adam step一致；29次跳步均有非有限梯度及scale减半记录。每千batch0.451713；旧14次历史更新差额0.495610/千batch，不等于14次已观测溢出。新旧≥10pp骤降为1/24与3/10个相邻转换；S42仍在5→6下降20.0118pp，不能宣称消除崩塌。60°recall提高20.8039pp但70°降低1.8106pp；互混45↔50由222降142、60↔70由839降458，70→60方向却176增226。

交付修正：原自动汇总S42 stop_reason误写historical reuse，仅文字标签；metadata证实max_epochs、early_stopped=false。原CSV/zip保留，分析派生表纠正，未来summarizer已修并本地47项测试通过。结果与选模不受影响，不重跑。完整中文报告`outputs/carbon_stability_20260916/analysis_final/report_zh.md`，给Astra的包`astra_carbon_stability_20260916.zip`置顶说明原则更改与历史对照限制。下一阶段尚未授权；不新增BN/AMP方案或结构搜索。

## 2026-09-16 碳离子T7差分参数化三seed队列

状态：本地适配、15项定向测试、独立代码复核及服务器两模型GPU检查通过。六项训练于2026-09-16 05:28:40+08完成，各run及队列退出码均为0；Rawls已完成115文件/1,448,620,831bytes的双端SHA核验，包含全部12个checkpoint。290项检查通过，1919项数值对照最大差2.22e-16，无需修改原summary。服务器训练提交`47dc2e0`，本轮无中断、恢复、重跑或新增训练，旧结果及回传原件不覆盖。

| 方法 | n | Val Acc | Val Balanced Acc | Val Macro-F1 | Val Weighted-F1 | Val argmax MAE / ° | P90 / ° |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| R42普通卷积，历史复用 | 1 | 88.1321% | 91.2522% | 0.913131 | 0.878776 | 1.115739 | 5 |
| A CDC | 3 | 93.8260±0.1091% | 95.4650±0.1063% | 0.954627±0.001032 | 0.938196±0.001049 | 0.572978±0.008116 | 0±0 |
| B 混合APDC | 3 | 93.7345±0.1623% | 95.4293±0.1119% | 0.954051±0.001175 | 0.937380±0.001594 | 0.582451±0.012409 | 0±0 |

先逐run后mean±sample std(ddof1)；R42的std留空，不复制为三条。主配对B−A的MAE三值为+0.018130/−0.012250/+0.022540°，均差+0.009473±0.018942°，B改善1/3 seed；Acc均差−0.091467±0.250685pp。CDC主要指标均值略优、离散程度较小，2/3配对占优，但APDC在seed43更好，不宣称显著胜负或稳定APDC优势。两种新模型均值相对旧R42的MAE参考差−0.542761/−0.533288°，仍只是历史单seed参照。

类别结果：60↔70错误R42=839，A=412.67±6.66、B=419.33±12.90；45↔50为222、136.00±6.08、137.33±3.79。B的60°recall三seed均提高，70°recall三seed均下降，不能只取前者声称全面提升。

执行及曲线限制：六项best/stop为24/25、17/25、21/25、24/25、20/25、23/25，B42恰在25轮触发patience8；其余达到上限，不代表充分收敛。无OOM/异常堆栈/非有限值，但B43 epoch8、B44 epoch6分别出现MAE2.533810/2.282438°的验证尖峰后恢复；六项末期Val loss较最低回升1.208–1.449倍。旧R42 best3/stop11且epoch9 MAE27.112407°，限制基线结构归因。只记录观察，不扩展BN/AMP诊断。所有本轮记录test_evaluated=false，但旧split声明test_is_new_untouched=false，不称独立全新测试。

最终处理：保留两种参数化的验证证据，CDC可作为下一次讨论的优先候选；本轮收口，不自动追加baseline种子、超参或V6。差分核不增加测量信息，不证明簇内物理梯度因果机制或近垂直可分。最终报告`outputs/carbon_difference_controls_20260915/final_review/experiment_report.md`，独立材料`analysis_review/`，轻量交付`carbon_difference_final_review.zip`。本地回传已核验，可安全关闭55870；Computer Use被URL安全检查停止，未关机，用户明确另开新对话处理。最终文档本地隔离分支提交推送，不改训练提交或借用SSH绕过关机限制。

关键决策：只跑T7，CDC(A)/混合APDC(B)各42/43/44，顺序A42→B42→A43→B43→A44→B44。复用旧R42验证成绩、原帧分组split与metadata标准化统计，不重训或加载旧权重初始化。训练保持原CE/Adam3e-4/wd1e-4/batch128/dropout0.1/25epoch/patience8/AMP，Val实际角度argmax MAE→Macro-F1→早epoch选模，test禁推理。四处layer1卷积换theta0.7差分运算，其余结构不变。

各run独立进程，同seed原始初始化和数据顺序一致；不设运行时限，不因成绩好坏停止后续项。正常早停算完成，外部中断不算完成；恢复须保留模型/优化器/调度器/RNG/早停状态，累计不超25轮。普通失败逐项记录并继续；共同GPU/数据/磁盘阻碍报告主控。无V6自动迁移、旧基线补种子、全量重审或超参网格。

主控本地隔离分支`codex/carbon-difference-55870`只提交本轮文件，不动主工作区其他任务。实验员Gibbs唯一执行/监控，审查员Pascal只读新代码，分析员Rawls在训练结束后回传与复算。完整训练/汇总/回传命令见`CARBON_DIFFERENCE_RUNBOOK.md`；详细范围与检查单见`CARBON_DIFFERENCE_PLAN.md`。R42仍n=1/std空，A/B三seed先逐run计算再汇总，主配对B_s−A_s，不把固定split三训练seed称为三独立数据集。

启动前审查修正：队列子进程继承OS锁，防止父进程被杀后重启造成双训练覆盖；信号退出记interrupted而非永久failed；单run目录识别异常（含空/截断YAML）记录后继续独立项；截断metadata不算完整完成，允许已有checkpoint收尾恢复。以上均为新队列错误处理修正，先构造失败测试再修复，不改变科学协议。仅对新改动进行15项测试；未重跑旧基线或全量旧链路审计。服务器虽缺optuna/pandas，但当前入口不依赖，未增加无关依赖。

## 2026-09-09 碳离子T7/V6四格完成与主控复核

状态：四个seed42 run均exit0，训练、完整checkpoint回传、双端SHA、逐验证事件指标/原帧/选模核验全部完成。主控查看四张完整训练曲线并复核汇总；本地全套25项测试通过。仅validation，test未评估。由Mill唯一执行训练，Sagan完成回传与只读分析，未追加训练、重跑、超参或结构搜索。

收尾连接异常：完整结果回传后，最终文档同步时39951 SSH握手拒绝连接，主控同样复现；已向用户询问实例状态。最终文档已在本地隔离分支提交并推送，服务器最后确认HEAD为`a8bbf56`，未声称已同步最终文档。断连前最后监控无训练进程、GPU空闲、数据盘约41.95GiB，不用旧快照替代当前服务器验收。不影响已回传且哈希核验完成的四组结果；没有擅自删除、关机或重启训练。

后续用户确认：服务器已主动关闭，“完成本地交付即可”。停止SSH重试，服务器最终文档拉取不再作为本轮完成前提；新增`final_review/server_shutdown_note.md`记录确认。四组训练和完整回传已在关闭前完成，不因本次拒绝连接判为训练中断。

| 格子 | Val Acc | Val Balanced Acc | Val Macro-F1 | Val Weighted-F1 | Val MAE / ° | P90 / ° | Best/Stop |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |
...
主控结论：T7幅值相对二值形状额外提供预测价值（ToT−Mask Acc+23.1870pp，MAE−2.598491°），但不证明梯度机制、不外推六个近垂直角度。V6-HiRes相对Base仅MAE小降0.074200°，Acc/BalAcc/F1均下降，两组MAE都差于训练集确定的常数84°参照2.920182°，本轮不支持将减少layer3/4下采样作为已验证有效改进。V6在线训练未充分拟合，不能据低分宣布物理不可分。

执行异常与解释边界：T7-ToT epoch2/6/8/9验证大幅下降，最佳虽好但不称稳定；Mask Val loss约0.7617–0.8357，未见同级别崩落。Base多轮验证退化，HiRes best1后8轮不改进。四组无观察到的OOM/代码异常/非有限日志指标。实际Adam步数ToT28234、Mask56472、Base95637、HiRes43037，分别比完成batch少14/24/43/19；BN计数与batch一致。与AMP跳步相容，但没有逐步证据定位原因，Mask平稳却也有差值，不把该差值直接归因为验证崩塌。实际fit秒依次739.54/1497.14/2406.93/2686.19，早停使步数不同；HiRes约2.48倍每轮计算时间。实验员后置校验一度误以为配对CSV只有一行，实际每项指标一行共六行，复核后通过，未修改或重生成原结果。

材料：旧准备报告不覆盖，新增`outputs/carbon_angle_controls_20260909/final_review/reuse_and_experiment_report.md`、`reuse_inventory.csv`、`four_run_validation.csv`、`run_manifest.csv`及`summary_all/`。四个独立分析目录保存传输/计算核验证据、逐类表、混淆与完整曲线；训练目录和可直接执行的汇总命令见`CARBON_CONTROLS_RUNBOOK.md`。轻量上传包不含原始事件或checkpoint，模型本地与服务器均保留。本轮到此停止，后续如需处理未拟合/验证震荡，另行讨论并冻结诊断方案。

## 2026-09-09 碳离子T7/V6正式启动授权

以下按发生顺序保留启动与阶段决策，均为历史时点状态。四格最终结果见上节，不将启动时的待运行状态当成当前状态。

用户在服务器验收完成后明确授权“开始训练”。沿用隔离分支和冻结协议，不改学习率、batch、epoch、选模规则、数据或划分。实验员Mill为唯一执行者，启动前核对GPU、tmux、进程、磁盘与Git状态；每个run独立记录日志和退出码，失败只反馈，不自主修改或重跑。

阶段顺序：先T7-ToT；完成后主控基于训练正确性、validation及机会参照判断是否放行T7-Mask；V6-Base和V6-HiRes另构成独立对照，随后单GPU串行运行。不得因为T7效果弱就自动取消V6。HiRes的batch128显存预检已通过。阶段状态：T7-ToT待启动，Mask待主控检查，V6两组待队列。

本轮仅validation评价，不输出test成绩。每个成功run回传完整结果含checkpoint，再由分析员只读分析；主控审核阶段决策、更新日志后再汇报。完整训练/汇总命令见`CARBON_CONTROLS_RUNBOOK.md`，环境与数据证据见`CARBON_SERVER_39951.md`。

阶段进展：T7-ToT在`20260909_014839_carbon_t7_tot_seed42`完成，exit0，best3/stop11，fit739.54秒；最佳Val Acc88.1321%、MAE1.115739°、Macro-F1 0.913131、P90 5°。epoch2/6/8/9出现明显验证下降，epoch6/8/9 Val loss约26.90/12.40/31.72；训练数值有限、无运行错误，但不能称全程稳定。保持原patience和预算，未重跑。分析员Sagan负责回传与预测/划分/选模核验。

执行顺序调整（不改科学协议）：T7分析与独立V6训练并行，由唯一实验员Mill先顺序执行V6-Base、V6-HiRes；T7-Mask在主控复核放行且GPU空闲后再运行。理由是V6结构对照不依赖七分类的稳定性结论；不增减配置、种子或预算，不同时运行两个训练进程。

T7复核与Mask放行：Sagan完成18个文件回传（含两个checkpoint），两端SHA全部一致；10204条验证预测、完整帧键、选模与指标独立复算通过，未评估test。未发现best被末轮覆盖、原帧交叉或预测错位。28248个训练batch对应28234次Adam更新，差14，与AMP跳步相容但未确定原因和发生轮次。多数类参照Acc28.1458%、中位角MAE14.9201°，T7最优表现明显优于二者。主控据此认定已有可学习信号且无已证实的正确性阻塞，批准Mask作为配对诊断，V6两组结束后再启动；不把此决策写成训练稳定或正常收敛，不追加调参或重跑。

V6-Base阶段核验：`20260909_020948_carbon_v6_base_seed42` exit0，best12/stop20，fit2406.93秒。14个回传文件SHA一致；18868条验证预测、完整帧键隔离、最佳权重与指标重算通过，test未评估。Val Acc19.0905%、Balanced Acc19.1702%、Macro-F1 0.138460、Weighted-F1 0.140690、MAE4.074730°、P90 8°。训练Acc仅从17.90%到19.81%，loss从1.78891到1.77801，未充分拟合；不能据此宣布图像不可分。最佳轮的80/82/84/86/88/90°预测数为3712/0/4204/1504/0/9448，82°和88°召回为0。训练集确定的多数类与中位角均为84°，验证MAE2.920182°，优于模型。95680个batch对应95637次Adam更新，差43次，不能仅凭此归因验证波动。多轮验证统计与固定预测80°或90°一致，但非最佳轮没有逐事件预测，不能冒充逐轮标签审计。HiRes已串行启动，不因Base接近机会水平而取消；维持冻结协议，无额外训练。
