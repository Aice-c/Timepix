...
- B3a/B3b Proton_C_7 有序损失实验已完成。B3a 固定 B1-best patience=8、ToT-only、无 handcrafted，只比较 loss / label strategy；seed42 选择 `CE+ExpectedMAE lambda=0.05` 作为 B3b-main，`CE+EMD lambda=0.05` 作为 optional 对照，Gaussian soft label 不继续推进。B3b 三 seed 结果显示 `CE+ExpectedMAE lambda=0.05` 是 Proton_C_7 当前推荐损失函数：B1-best CE onehot 为 Test Acc 93.26±1.64%、MAE 0.640±0.161、Macro-F1 0.952±0.011；B3b-main 提升到 Test Acc 94.38±0.08%、MAE 0.532±0.010、Macro-F1 0.960±0.001。`CE+EMD lambda=0.05` 结果非常接近，Val MAE 和 far-error rate 略优，但 primary validation accuracy / Macro-F1 不是主结果最优，因此保留为 ordered-loss 对照。该结果说明有序角度辅助损失比 B2 手工特征更适合 Proton_C_7 的剩余相邻大角度混淆问题。
