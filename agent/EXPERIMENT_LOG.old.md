...
| `A5` | 物理/手工特征融合 | A5a/A5b/A5c/A5d 已完成 | 聚焦 ToT image + selected ToT/ToA scalar physical features；A5a 完成传统模型/置换重要性筛选，A5b 完成低冗余 CNN concat pilot，A5c 显示 gated 优于 concat；A5d 三 seed 表明手工标量 gated 融合可改善 validation 指标和部分 MAE/F1，但未稳定提升 Test Acc。 |
| `A6` | Alpha 角度有序性损失与标签策略 | A6a 配置已撰写 | 作为 Alpha 版 B3，固定 A2 ToT baseline，仅比较 loss / label strategy；不混入 A5 handcrafted 或 A4 多模态结构。 |
...
| `A4b-5` | sample-wise gated late fusion | 已完成 | frozen-expert gate 比 hard rule 更好；test 泛化表现强，但不作为基于 test 反选的最终架构依据。 |
| `A4b-6` | residual gated fusion | 已完成 | validation-selected expert-level residual fusion；适合作为性能导向的后处理融合系统，但不是端到端多模态架构。 |
...
| `A5c` | handcrafted gated diagnostic | 镜像 A5b 的 4 个低冗余特征组，只把融合方式改为 `gated`，检查 gated 是否能改善 simple concat 的弱/负结果 | 已完成 seed42；gated 在 4 个特征组上均较 concat 提高 Test Acc/MAE，其中五维 `geometry+ToT+ToA` 最好。 |
| `A5d` | best handcrafted fusion 3-seed verification | 对 A5c 选出的 main/diagnostic 设置做 `training.seed=42/43/44` 认证 | 正式配置已撰写；main 为五维 `geometry+ToT+ToA gated`，diagnostic 为 `ToA-only gated`。 |
...
- A5c: 已完成 4 个 seed42 run，逐一对应 A5b 的 4 个特征组。
- A5d: 正式安排为 6 个 run，包含 2 个 feature groups × 3 seeds；main 为五维 `geometry+ToT+ToA gated`，diagnostic 为 `ToA-only gated`。
...
- A5d 正式配置已新增：`configs/experiments/a5d_alpha_handcrafted_gated_3seed.yaml`。旧 `a5d_alpha_handcrafted_best_3seed_TEMPLATE.yaml` 继续保留为未来单设置认证模板。
...
B3b-main 使用 `CE+ExpectedMAE lambda=0.05` 做三 seed 认证；B3b-optional 使用 `CE+EMD lambda=0.05` 作为有序损失对照。两组当前均已完成。
...
| 设置 | Delta Val Acc | Delta Val MAE | Delta Val F1 | Delta Test Acc | Delta Test MAE | Delta Test F1 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| CE+ExpectedMAE lambda=0.05 | +1.26 pp | -0.126 | +0.009 | +1.12 pp | -0.109 | +0.008 |
| CE+EMD lambda=0.05 | +1.26 pp | -0.131 | +0.008 | +1.09 pp | -0.108 | +0.008 |

误差结构摘要：

| 设置 | Val 45<->50 | Val 60<->70 | Val far>=20 deg | Test 45<->50 | Test 60<->70 | Test far>=20 deg |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| B1-best CE | 128.7±11.6 | 484.7±155.6 | 0.217±0.048% | 122.0±17.3 | 471.7±134.2 | 0.181±0.039% |
| CE+ExpectedMAE lambda=0.05 | **119.0±1.7** | 392.7±4.5 | 0.198±0.015% | **107.0±2.6** | **386.0±12.0** | 0.142±0.015% |
| CE+EMD lambda=0.05 | 123.0±0.0 | **383.0±14.0** | **0.168±0.039%** | 110.3±3.8 | 387.3±11.7 | **0.139±0.037%** |

当前结论：

- `CE+ExpectedMAE lambda=0.05` 是 Proton_C_7 当前推荐损失函数。它是 B3a 按 validation accuracy 选出的主配置，并且在 B3b 三 seed 中保持 Val Acc、Val Macro-F1、Val High-angle F1 最优。
- `CE+EMD lambda=0.05` 作为有序损失对照保留。它的 Val MAE 稍好，远距离错误率也略低，但 primary metric 不是最优，因此不替代 B3b-main。
- B3b 是强正结果：相比 B1-best，`CE+ExpectedMAE lambda=0.05` 同时提升 accuracy、降低 MAE、提高 Macro-F1，并显著降低 seed 间波动；这说明有序角度辅助损失比继续堆手工特征更适合 Proton_C_7 的剩余相邻角度混淆问题。
- 后续 Proton_C_7 最终模型确认（B4）应以 B1-best 结构/训练配置 + `CE+ExpectedMAE lambda=0.05` 为默认候选，`CE+EMD lambda=0.05` 作为论文中的 ordered-loss 对照。
...
| 方法 | 15 deg R/F1 | 30 deg R/F1 | 45 deg R/F1 | 60 deg R/F1 |
| --- | ---: | ---: | ---: | ---: |
| A2 seed42 | 0.821 / 0.763 | 0.297 / 0.402 | 0.754 / 0.751 | 0.712 / 0.669 |
| A5c `geometry_lowcorr` | 0.852 / 0.774 | 0.193 / 0.301 | 0.777 / 0.764 | 0.718 / 0.673 |
| A5c `geometry_plus_tot_lowcorr` | 0.860 / 0.769 | 0.228 / 0.340 | 0.763 / 0.758 | 0.692 / 0.673 |
| A5c `toa_lowcorr_diagnostic` | 0.792 / 0.743 | 0.283 / 0.366 | 0.785 / 0.766 | 0.686 / 0.682 |
| A5c `geometry_plus_tot_plus_toa_lowcorr` | 0.801 / 0.755 | 0.303 / 0.386 | 0.799 / 0.774 | 0.686 / 0.693 |

阶段判断：

- A5c 说明 `gated` 比 A5b 的 simple concat 更适合这批低维手工特征；4 个特征组相对对应 A5b concat 都提升 Test Acc，且 MAE 均下降。
- A5c 最好设置是五维 `geometry_plus_tot_plus_toa_lowcorr_gated`：Test Acc 71.07%，MAE 5.651，Macro-F1 0.652，是当前 A5 的最佳 CNN+handcrafted seed42 结果。
- 该提升仍是小幅正结果，不是决定性突破；相对 A2 seed42，Test Acc +0.60 pp，MAE -0.313，Macro-F1 +0.006。
- A5c 的主要改善在 `45 deg` 和 `60 deg`，没有真正解决最困难的 `30 deg`；五维 gated 的 30 deg F1 为 0.386，仍低于 A2 seed42 的 0.402。
- 后续进入 A5d 三 seed 认证。为避免按 seed42 test 指标单独挑选模型，A5d 同时保留两组设置：五维 `geometry_plus_tot_plus_toa_lowcorr_gated` 作为 main，原因是它是物理上最完整的低冗余组合，且在 seed42 同时改善 Test Acc、MAE 和 Macro-F1；`toa_lowcorr_diagnostic_gated` 作为 diagnostic，原因是它是 A5c seed42 validation accuracy 最高的 ToA-only 标量组，但 Test Acc/Macro-F1 不优，不能作为主结果。

## A5d Alpha handcrafted gated 3-seed verification

记录日期：2026-04-30。

阶段目的：

- 对 A5c 中最值得继续的手工标量 `gated` 融合设置做三 seed 认证。
- 检查五维低冗余物理标量组是否能稳定补充 `Alpha_100 + ToT` CNN。
- 同时保留 ToA-only 标量诊断组，避免只根据 seed42 test 最优组做单一路径确认。
...
- A5d-main 不是因为 seed42 test 单项最高才进入三 seed，而是因为它是预先规划的完整低冗余物理组，并且在 A5c seed42 中同时改善 Test Acc、MAE 和 Macro-F1。
- A5d-diagnostic 保留 ToA-only gated，是因为它在 A5c seed42 中 Val Acc 最高，用于检查 ToA 标量信号是否稳定；即使它最终不提升 test，也能支持“单独 ToA 标量不够稳定”的论文表述。
- 当前配置不扩展到 12 维或 25 维特征池，也不做逐特征开关网格，避免 A5 变成 test-driven feature engineering。
- A5d 的 `dataset.modalities` 必须保持 `[ToT]`，`handcrafted_features.source_modalities` 才允许 `[ToT, ToA]`；这样模型不会看到 ToA 图像通道，只会使用 ToA 标量。
...
- A5d 共 6 个 run，即 2 个 feature groups × 3 个 training seeds。
- `summarize.py` 输出逐 run 汇总，`aggregate_seeds.py` 输出 mean/std。
- 当前 A5d 配置只负责训练与标准汇总；如果后续需要 validation-set feature permutation 或更细的 handcrafted gate 统计，应另加诊断脚本，不能用 test set 做特征选择。
- 本地 Windows dry-run 已通过，使用 `--data-root D:\Project\Timepix\Data\Alpha_100` 时规划 6 个 run，与预期一致。
...
| 组别 | Seed | Best/Stop | Val Acc | Val MAE | Val F1 | Test Acc | Test MAE | Test F1 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `main_5feat` | 42 | 21/25 | 70.53% | 6.129 | 0.647 | 71.07% | 5.651 | 0.652 |
| `main_5feat` | 43 | 20/25 | 70.23% | 6.204 | 0.660 | 69.18% | 6.113 | 0.644 |
| `main_5feat` | 44 | 24/25 | 69.93% | 6.294 | 0.645 | 70.28% | 5.860 | 0.643 |
| `toa_only_diag` | 42 | 23/25 | 71.03% | 6.084 | 0.657 | 69.98% | 5.964 | 0.639 |
| `toa_only_diag` | 43 | 18/25 | 70.63% | 6.159 | 0.646 | 70.28% | 5.964 | 0.640 |
| `toa_only_diag` | 44 | 21/25 | 70.43% | 6.144 | 0.649 | 69.88% | 5.949 | 0.644 |

相对 A2 ToT baseline：

| 设置 | ΔVal Acc | ΔVal MAE | ΔVal F1 | ΔTest Acc | ΔTest MAE | ΔTest F1 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| `main_5feat` | +1.20 pp | -0.215 | +0.029 | -0.27 pp | -0.075 | +0.010 |
| `toa_only_diag` | +1.67 pp | -0.295 | +0.029 | -0.40 pp | +0.010 | +0.005 |

关键结论：

- 若严格按照 `validation accuracy` 选择 A5d 内部最佳设置，应选择 `toa_only_diag`，不是 `main_5feat`。
- `main_5feat` 的 Test MAE 和 Test Macro-F1 最好，但不能反过来作为模型选择依据；它只能作为诊断说明五维物理组对误差幅度和类别均衡有一定补充。
- 两组 A5d 都没有超过 A2 ToT baseline 的 Test Acc，因此 A5 不应表述为“手工特征稳定提升总体准确率”。
- 更稳妥论文口径：低维手工标量具有独立物理判别力，`gated` 融合比 simple concat 更合适；但在 ResNet18 已经充分利用 ToT 图像结构后，显式标量特征主要带来 validation 和 MAE/F1 层面的有限补充，Test Acc 增益不稳定。
...
- A6 定义为 Alpha 版 B3，聚焦角度类别有序性相关的 loss / label strategy。
- 固定模型结构、输入模态、训练超参和 split，只比较损失函数与标签策略是否能降低角度误差、改善 Macro-F1 和 `30 deg` 困难类别。
- A6 不继续扩展 A5 handcrafted feature 组合，也不重新引入 A4 多模态架构大网格。
...
- B3b 已确认 `CE+ExpectedMAE lambda=0.05` 在 Proton_C_7 上是强正结果，`CE+EMD lambda=0.05` 是接近的 ordered-loss 对照；A6a 因此继续优先比较 `ce_expected_mae` 与 `ce_emd` 的轻量权重，不新增 pure EMD 或复杂 regression head。
...
- A6c 如果使用 `dual_stream_gmu_aux`，当前 trainer 会把同一个 criterion 同时作用于 main logits 与 auxiliary heads，再乘 `weight_tot/weight_toa`。该实现可接受，但论文与日志中需说明；暂不引入 “main 用 ordinal loss、aux 仍用 CE” 的额外 trainer 分支。
...
- A6a 只做 seed42 screening，不作为最终三 seed 结论。
- 模型选择只使用 validation，不使用 test。
- Primary: `Val Acc`
- Tie-break 1: `Val MAE`
- Tie-break 2: `Val Macro-F1`
- 额外关注：`30 deg` recall/F1、`far_error_rate_abs_ge_20`、confusion matrix。
...
| `B3` | Proton_C_7 损失/近角度分类策略 | B3a 与 B3b 已完成 | B3b 证明 `CE+ExpectedMAE lambda=0.05` 是当前推荐损失：Test Acc 93.26±1.64% -> 94.38±0.08%，MAE 0.640±0.161 -> 0.532±0.010；`CE+EMD lambda=0.05` 保留为有序损失对照。 |
...
2. A4 系列最终分层：`A4b-6` 作为 validation-selected expert-level 后处理融合系统；`A4c-2 dual_stream_gmu_aux` 作为论文主推的端到端 ToT/ToA 多模态架构。GMU 的选择依据来自 validation 侧均衡指标、Val MAE、稳定性和 gate 物理解释，不能用 test 结果反选。
3. A4c 第一批 `A4c-1/2/3` 已完成；第二批 `A4c-4 warm_started_expert_gate` 也已完成；`A4c-5 mmtm_lite` 仍为选做，但当前多模态结构探索建议收束，不优先继续扩展新架构。
...
- A4c 端到端架构选择不能使用 test 结果反选。正式论文口径应基于 validation 侧指标和结构解释：`dual_stream_gmu_aux` 的 Val Acc 接近 FiLM，Val Macro-F1 与 FiLM 近似持平，Val MAE 在端到端 A4c 模型中最好，且 gate 机制与 A4b “ToA 选择性辅助” 的结论一致，因此可作为最终端到端多模态架构。
...
核心决策：最终多模态结果分成两个层面，而不是把 A4b 和 A4c 混成单一 winner。

| 层面 | 推荐对象 | 选择依据 | 论文定位 |
| --- | --- | --- | --- |
| expert-level 后处理融合系统 | `A4b-6 residual gated fusion` | validation-selected residual fusion 的 Val Acc / Val MAE 最强 | 性能导向的 frozen-expert 后处理融合系统 |
| 端到端多模态神经网络架构 | `A4c-2 dual_stream_gmu_aux` | A4c 内部 Val Macro-F1 与 FiLM 接近、Val MAE 最好、稳定性好，并且 gate 机制与物理解释一致 | 论文主推 final end-to-end ToT/ToA multimodal architecture |

A4c 架构选择规则：

```text
1. 不使用 test 结果选择最终架构。
2. 优先看 validation Macro-F1。
3. Macro-F1 差异小于标准差量级时，看 validation MAE。
4. 再看稳定性、是否有可解释 gate / modulation 诊断，以及是否符合 A4b 得到的物理结论。
```

GMU 的 validation 侧选择理由：

```text
dual_stream_gmu_aux:
  Val Acc      = 70.20 ± 0.67%
  Val MAE      = 6.274 ± 0.129
  Val Macro-F1 = 0.668 ± 0.007

toa_conditioned_film:
  Val Macro-F1 = 0.669 ± 0.011
```

FiLM 的 Val Macro-F1 只比 GMU 高 0.001，远小于 std，不能认为 FiLM 明显优于 GMU。GMU 的 Val MAE 更好，std 也更小；同时 GMU 的显式 gate 更直接对应 “ToT 是主模态、relative ToA 是选择性辅助模态” 的 A4b 结论。

test 结果只能用于泛化报告，不用于选择。可以写作：

```text
After selecting GMU based on validation-side balanced metrics and model interpretability,
test-set results were used only for final generalization reporting.
```

论文建议表述：

> 本文最终采用双分支 `ResNet18_no_maxpool + auxiliary heads + GMU gated fusion` 作为端到端 ToT/ToA 多模态架构。该模型不是由 test 指标反选得到，而是在 validation Macro-F1、validation MAE、训练稳定性、gate 可解释性和物理一致性之间取得了最合适的平衡。`A4b-6 residual gated fusion` 则作为 validation-selected expert-level 融合系统，用于说明 frozen-expert 后处理融合在 accuracy/MAE 口径下仍有优势。
