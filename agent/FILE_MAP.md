...
| `configs/experiments/b1_proton_c7_resnet18_tot_best_3seed.yaml` | B1-best Proton_C_7 early-stop diagnostic | Older 3-seed verification with `early_stopping_patience=5`; retained to document why patience was too aggressive. |
| `configs/experiments/b1_proton_c7_resnet18_tot_best_patience8_3seed.yaml` | Formal B1-best Proton_C_7 3-seed verification | Official Proton_C_7 CE baseline with B1-2 best hyperparameters and `early_stopping_patience=8`; supersedes the patience-5 diagnostic. |
...
| `configs/experiments/a6a_alpha_tot_ordinal_loss_seed42.yaml` | A6a Alpha ordered-loss screening | Completed seed42 Alpha_100 ToT loss/label screening aligned with B3; validation-selected candidate is `CE+EMD lambda=0.02`, but the gain is weak/tie-break level and CE one-hot is reused from A2-best. |
| `configs/experiments/a6b_alpha_tot_ce_emd_0p02_3seed.yaml` | A6b Alpha ordered-loss verification | Completed three-seed verification of A6a validation-selected `CE+EMD lambda=0.02`; result is weaker and less stable than A2 CE baseline, so A6c is not pursued. |
