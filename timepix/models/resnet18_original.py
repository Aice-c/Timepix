"""Original-stem ResNet18 Timepix baseline."""
...
class ResNet18OriginalBackbone(nn.Module):
    """ResNet18 with the original 7x7/stride-2/maxpool stem.

    The only stem adaptation is input channel count, so single-channel ToT and
    two-channel ToT+ToA tensors can use the same baseline.
    """
...
class ResNet18OriginalTimepix(nn.Module):
    def __init__(
        self,
        input_channels: int,
        num_classes: int,
        ...
        super().__init__()
        self.task = task
        self.backbone = ResNet18OriginalBackbone(input_channels, feature_dim, pretrained)
